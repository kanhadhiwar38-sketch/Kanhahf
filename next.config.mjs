...
const nextConfig = {
  ...
  async rewrites() {
    // Frontend/backend split: when BACKEND_URL is set (e.g. on the Vercel
    // frontend deployment), every /api/* call is proxied to the standalone
    // backend (Docker on Railway/Fly/VPS) instead of being handled locally.
    // Leave BACKEND_URL unset when deploying the full app (backend or
    // single-deployment mode) and these routes resolve locally as before.
    const backendRewrites = process.env.BACKEND_URL
      ? [
          {
            source: "/api/:path*",
